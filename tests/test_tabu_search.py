...
async def test_local_Local_search():
    ...
    with AxoContextManager.local() as dcm:
        ...
        res = ts.tabu()
        assert res.is_ok
        best ,a,history = res.unwrap()
        print("Tabu Search:", best,a,history)


@pytest.mark.asyncio
async def test_distributed_tabu_search(endpoint_manager:DistributedEndpointManager,storage_service:MictlanXStorageService):
    with AxoContextManager.distributed(endpoint_manager=endpoint_manager,storage_service=storage_service) as dcm:
        ...
        ts.append_dependency("matplotlib==3.9.2")
        
        persistify_res = await ts.persistify()
        
        assert persistify_res.is_ok
...
@pytest.mark.asyncio
async def test_distributed_tabu_search_zk(endpoint_manager:DistributedEndpointManager,storage_service:MictlanXStorageService):
    with AxoContextManager.distributed(endpoint_manager=endpoint_manager,storage_service=storage_service) as dcm:
